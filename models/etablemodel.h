...
public:
    ...
    int getHeaderPosition(QVariant hdrtext, Qt::Orientation orientation,
        int role);                         // получение индекса элемента в заголовке,
                                           // который содержит текст hdrtext
    void addColumn(const QString hdrtext); // добавление новой колонки с текстом в заголовке
                                           // hdrtext для варианта двух столбцов
    ...
    bool isEmpty();
    void SetColumnFormat(int column,
        int format); // format is precision of the double, set
                     // num above 10 to set no format
