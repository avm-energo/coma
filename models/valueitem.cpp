...
ValueItem::ValueItem(const ValueItem &item)
    : m_name(item.m_name), m_tooltip(item.m_tooltip), m_style(item.m_style), m_format(item.m_format)
{
    switch (m_style)
    {
    case DataStyles::ComboBox:
    {
        m_addData = newVar<QStringList>(item.m_addData);
    ...
    case DataFormat::OUTVALUEDOUBLE:
        m_dataPtr = newVar<double>(item.m_dataPtr);
        break;
...
void *ValueItem::dataPtr() { return m_dataPtr; }

void ValueItem::setDataPtr(void *ptr) { m_dataPtr = ptr; }

QString ValueItem::name() { return m_name; }

void ValueItem::setName(const QString &str) { m_name = str; }

QString ValueItem::tooltip() { return m_tooltip; }

void ValueItem::setTooltip(const QString &str) { m_tooltip = str; }

ValueItem::DataStyles ValueItem::style() { return m_style; }

void ValueItem::setStyle(ValueItem::DataStyles stl) { m_style = stl; }

ValueItem::DataFormat ValueItem::format() { return m_format; }

void ValueItem::setFormat(ValueItem::DataFormat fmt) { m_format = fmt; }

void *ValueItem::addData() { return m_addData; }

void ValueItem::setAddData(void *ptr) { m_addData = ptr; }
