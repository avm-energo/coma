...
#define CN_MINOSCID                                                                                                    \
    1000 // минимальный ИД осциллограмм, нужно, т.к. файлы осциллограмм
...
public:
    ...
    virtual QStringList DevicesFound() = 0; // функция, возвращающая список найденных устройств
                                            // (COM-портов, устройств USB)
    void TranslateDeviceAndSave(const QString &str); // функция, разбивающая строку устройства и
                                                     // складывающая в соотв. структуру

signals:
    void SetDataSize(int); // сигналы для прогрессбаров - отслеживание принятых
                           // данных, стёртых осциллограмм и т.п.
...
private:
    ...
    bool LastBlock; // признак того, что блок последний, и больше запрашивать не
                    // надо
    char BoardType;
    ...
    void SendErr();
    bool GetLength(); // ok = 1 -> обработка посылки вида SS OK ..., ok = 0 -> вида SS
                      // c L L ... возвращаемое значение = false -> неправильная длина
};
