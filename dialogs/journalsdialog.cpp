...
void JournalDialog::SetupUI() {
  ...
  setStyleSheet(tmps);
  QString ConfTWss =
      "QTabBar::tab:selected {background-color: " + QString(TABCOLOR) + ";}";
  ...
  */
  ...
  if (StdFunc::IsInEmulateMode()) {
    ...
    WDFunc::SetEnabled(this, "gj." + QString::number(Journals::JOURMEAS),
                       false);
    WDFunc::SetEnabled(this, "gj." + QString::number(Journals::JOURWORK),
                       false);
    WDFunc::SetEnabled(this, "ej." + QString::number(Journals::JOURSYS), false);
    WDFunc::SetEnabled(this, "ej." + QString::number(Journals::JOURMEAS),
                       false);
    WDFunc::SetEnabled(this, "ej." + QString::number(Journals::JOURWORK),
                       false);
    WDFunc::SetEnabled(this, "sj." + QString::number(Journals::JOURSYS), false);
    WDFunc::SetEnabled(this, "sj." + QString::number(Journals::JOURMEAS),
                       false);
    WDFunc::SetEnabled(this, "sj." + QString::number(Journals::JOURWORK),
                       false);
  }
}
...
QWidget *JournalDialog::JourTab(int jourtype) {
  ...
  hlyout->addWidget(WDFunc::NewPB(this, "gj." + QString::number(jourtype),
                                  "Получить " + str, this,
                                  SLOT(TryGetJourByUSB())));
  hlyout->addWidget(WDFunc::NewPB(this, "ej." + QString::number(jourtype),
                                  "Стереть " + str, this, SLOT(EraseJour())));
  hlyout->addWidget(WDFunc::NewPB(this, "sj." + QString::number(jourtype),
                                  "Сохранить журнал в файл", this,
                                  SLOT(SaveJour())));
...
void JournalDialog::SaveJour() {
  ...
  jourfilestr += QString::number(MTypeB, 16) + QString::number(MTypeM, 16) +
                 " #" +
                 QString("%1").arg(ModuleBSI::SerialNum(BoardTypes::BT_MODULE),
                                   8, 10, QChar('0')) +
                 " ";
  jourfilestr += QDate::currentDate().toString("dd-MM-yyyy") + ".xlsx";
  // запрашиваем имя файла для сохранения
  QString filename = Files::ChooseFileForSave(
      nullptr, "Excel documents (*.xlsx)", "xlsx", jourfilestr);
  WW = new WaitWidget;
...
int JournalDialog::GetJourNum(const QString &objname) {
  ...
  if (((jourprefix != "gj") && (jourprefix != "ej") && (jourprefix != "sj") &&
       (jourprefix != "mj")) ||
      !ok) {
    DBGMSG;
    return GENERALERROR;
...
int JournalDialog::WriteCheckPassword() {
  ...
  connect(dlg, SIGNAL(Finished(QString)), this,
          SLOT(WritePasswordCheck(QString)));
