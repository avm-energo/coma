...
TuneDialogKTF::TuneDialogKTF(QVector<S2::DataRec> *S2Config, QWidget *parent) : EAbstractTuneDialog(parent)
{
    this->S2ConfigForTune = S2Config;
    int i;

    CKTF = new ConfigKTF(*S2ConfigForTune);
...
QHBoxLayout *TuneDialogKTF::MipPars(int parnum, const QString &groupname)
{
    QHBoxLayout *hlyout = new QHBoxLayout;
    QString ValuesFormat = "QLabel {border: 1px solid green; border-radius: 4px; padding: 1px; color: black;"\
            "background-color: "+QString(ACONFOCLR)+"; font: bold 10px;}";
...
void TuneDialogKTF::SetPf()
{
    int count = 0;
    pf[lbls.at(count++)] = &EAbstractTuneDialog::CheckPassword; // Ввод пароля
    int (EAbstractTuneDialog::*func)() = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::SaveWorkConfig); // Сохранение текущей конфигурации
    pf[lbls.at(count++)] = func; // 2. Отображение схемы подключения
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::ShowControlChooseDialog); // Отображение диалога выбора режима контроля показаний
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Show3PhaseScheme); // Отображение схемы подключения
    pf[lbls.at(count++)] = func;
    //func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_2_3); // Проверка связи РЕТОМ и МИП
    //pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_1); // Информация
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::SetNewTuneCoefs); // Установка коэффициентов
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_2); // Получение текущих аналоговых данных
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4); // Информация
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_2); // Расчёт коррекции по фазе
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_6); // Расчёт коррекции по частоте
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_7); // Конфигуратор рассчитывает новые значения калибровочных коэффициентов по току для Кацп=2
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_11); // Получение текущих аналоговых данных
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_12); // Расчёт коррекции взаимного влияния каналов
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_13); // Получение текущих аналоговых данных и расчёт настроечных коэффициентов по напряжениям
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_14); // Сохранение конфигурации
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_15); // Получение текущих аналоговых данных
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_16); // Ввод измеренных значений
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_17); // Расчёт настроечных коэффициентов по токам, напряжениям и углам
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_4_18); // Сохранение конфигурации
    pf[lbls.at(count++)] = func;
    func = reinterpret_cast<int (EAbstractTuneDialog::*)()>(&TuneDialogKTF::Start7_3_5); // Восстановление сохранённой конфигурации и проверка
    pf[lbls.at(count++)] = func;
}
...
void TuneDialogKTF::FillBac(int bacnum)
{
    Q_UNUSED(bacnum);
    for (int i = 0; i < 3; i++)
    {
...
void TuneDialogKTF::FillBackBac(int bacnum)
{
    Q_UNUSED(bacnum);
    QString tmps;
    for (int i = 0; i < 3; i++)
...
void TuneDialogKTF::SetDefCoefs()
{
...
int TuneDialogKTF::SetNewTuneCoefs()
{
...
float TuneDialogKTF::ToFloat(QString text)
{
    bool ok;
    float tmpf;
...
void TuneDialogKTF::ParseMipData(Parse104::FlSignals104 &Signal)
{
    // precision
...
bool TuneDialogKTF::IsWithinLimits(double number, double base, double threshold)
{
...
void TuneDialogKTF::SetTuneMode()
{
    TuneControlType = sender()->objectName().toInt();
}

void TuneDialogKTF::StopMip()
{
    emit stopall();
}
...
int TuneDialogKTF::ShowControlChooseDialog()
{
...
int TuneDialogKTF::Show3PhaseScheme()
{
    //QDialog *dlg = new QDialog;
    //QVBoxLayout *lyout = new QVBoxLayout;
    //QPixmap pmp;
...
int TuneDialogKTF::Start7_3_2()
{
   ...
   FillBac(0);
    ...
    //for(i = 0; i<3; i++)
    //CKTF->Bci_block.C_pasp[i] = 15036;
    CKTF->Bci_block.Imax = 7.8;
    CKTF->Bci_block.Umax = 74;
...
int TuneDialogKTF::Start7_3_4_2()
{
    QGridLayout *glyout = new QGridLayout;
    QVBoxLayout *vlyout = new QVBoxLayout;
...
int TuneDialogKTF::Start7_3_4_6()
{
    //int i;
    //for(i = 0; i<3; i++)
    //CKTF->Bci_block.C_pasp[i] = 10024;
    CKTF->Bci_block.Imax = 31;
    CKTF->Bci_block.Umax = 290;
...
int TuneDialogKTF::Start7_3_4_7()
{
...
int TuneDialogKTF::Start7_3_4_11()
{
    //int i;
    //for(i = 0; i<3; i++)
    //CKTF->Bci_block.C_pasp[i] = 5012;
    CKTF->Bci_block.Imax = 15.5;
    CKTF->Bci_block.Umax = 145;

    if (Commands::WriteFile(1, S2ConfigForTune) != NOERROR) //&CKTF->Bci_block,
    return GENERALERROR;
...
int TuneDialogKTF::Start7_3_4_12()
{
...
int TuneDialogKTF::Start7_3_4_13()
{
    //int i;
    //for(i = 0; i<3; i++)
    //CKTF->Bci_block.C_pasp[i] = 2506;
    CKTF->Bci_block.Imax = 3.9;
    CKTF->Bci_block.Umax = 300;
...
int TuneDialogKTF::Start7_3_4_14()
{
...
int TuneDialogKTF::Start7_3_4_15()
{
    //int i;
    //for(i = 0; i<3; i++)
    CKTF->Bci_block.Imax = 1.9;
    CKTF->Bci_block.Umax = 300;
...
int TuneDialogKTF::Start7_3_4_16()
{
...
int TuneDialogKTF::Start7_3_4_17()
{
    //int i;
    //for(i = 0; i<3; i++)
    //CKTF->Bci_block.C_pasp[i] = 626;
    CKTF->Bci_block.Imax = 0.9;
    CKTF->Bci_block.Umax = 300;
...
int TuneDialogKTF::Start7_3_4_18()
{
...
int TuneDialogKTF::GetExternalData()
{
#if PROGSIZE != PROGSIZE_EMUL
    ...
    case TUNEMAN:
    {
        ...
        if (GED_Type & TD_GED_I)
        {
            for (int i=0; i<3; i++) // for A to C
            {
                ...
                spb->setValue(CKTF->Bci_block.Imax);//(1.5*CKTF->Bci_block.C_pasp[i]*CKTF->Bci_block.Unom*qPow(10,-6)*CONST2PIF);
                glyout->addWidget(spb,row, column++,1,1);
            }
            ++row;
            column = 0;
    ...
    case TUNERET:
    {
        ...
        {
            RealData.u[i] = 60.0;
            RealData.i[i] =(CKTF->Bci_block.Imax);//(1.5*CKTF->Bci_block.C_pasp[i]*CKTF->Bci_block.Unom*qPow(10,-6)*CONST2PIF);
            RealData.d[i] = 0.0;
            RealData.f[i] = 50.0;
        }
        return NOERROR;
...
void TuneDialogKTF::EnterDataTune()
{
    int i;
...
void TuneDialogKTF::Enter20Data()
{
    int i;
    ask = new QDialog(this);
    //QVBoxLayout *lyout = new QVBoxLayout;
...
void TuneDialogKTF::Enterminus20Data()
{
    int i;
    ask = new QDialog(this);
    //QVBoxLayout *lyout = new QVBoxLayout;
...
void TuneDialogKTF::Enter60Data()
{
    int i;
    ask = new QDialog(this);
    //QVBoxLayout *lyout = new QVBoxLayout;
...
void TuneDialogKTF::CloseAsk()
{
    ask->close();
}

void TuneDialogKTF::SaveValuesTemp20()
{
    int i;
    QString tmps;
    for (i=0; i<3; i++)
...
void TuneDialogKTF::SaveValuesTempMinus20()
{
    int i;
    QString tmps;
    for (i=0; i<3; i++)
...
void TuneDialogKTF::SaveValuesTemp60()
{
    int i;
    QString tmps;
    for (i=0; i<3; i++)
...
int TuneDialogKTF::CalcTuneCoefs()
{
    int i;
    QString tmps;
    for (i=0; i<3; i++)
...
/*int TuneDialogKTF::CalcTuneCoefsKadc4()
{

    return NOERROR;
}

int TuneDialogKTF::CalcTuneCoefsKadc8()
{

    return NOERROR;
}

int TuneDialogKTF::CalcTuneCoefsKadc16()
{

    return NOERROR;
}

int TuneDialogKTF::CalcTuneCoefsKadc32()
{

    return NOERROR;
}*/

void TuneDialogKTF::GetBdAndFillMTT()
{

}

int TuneDialogKTF::SaveWorkConfig()
{
    if (Commands::GetFileWithRestore(CM_CONFIGFILE,S2ConfigForTune) == NOERROR)
        memcpy(&Bci_block_work,&CKTF->Bci_block,sizeof(ConfigKTF::Bci));
    else
        return GENERALERROR;
    return NOERROR;
}

int TuneDialogKTF::ReadAnalogMeasurements()
{
    int i,j;
    float sumU[3];
    float sumI[3];
...
int TuneDialogKTF::ReadAnalogTemp20()
{
    int i,j;
    float sumU[3];
    float sumI[3];
...
int TuneDialogKTF::ReadAnalogTemp60()
{
    int i,j;
    float sumU[3];
    float sumI[3];
...
int TuneDialogKTF::ReadAnalogTempMinus20()
{
    int i,j;
    float sumU[3];
    float sumI[3];
...
int TuneDialogKTF::ShowRetomDialog(double U, double I, double Y)
{
    QDialog *dlg = new QDialog;
    QVBoxLayout *lyout = new QVBoxLayout;
...
/*int TuneDialogKTF::ShowRetomDialog2(double U)
{
    QDialog *dlg = new QDialog;
    QVBoxLayout *lyout = new QVBoxLayout;
...
int TuneDialogKTF::LoadWorkConfig()
{
    // пишем ранее запомненный конфигурационный блок
    memcpy(&CKTF->Bci_block,&Bci_block_work,sizeof(ConfigKTF::Bci));
    if (Commands::WriteFile(CM_CONFIGFILE, S2ConfigForTune) != NOERROR)
        return GENERALERROR;
    return NOERROR;
}

void TuneDialogKTF::SetExtData()
{
...
void TuneDialogKTF::CancelExtData()
{
    QDialog *dlg = this->findChild<QDialog *>("dlg7371");
    if (dlg == nullptr)
        return;
...
QWidget *TuneDialogKTF::Bd1W(QWidget *parent)
{
    int i;
...
void TuneDialogKTF::FillBd1(QWidget *parent)
{
...
void TuneDialogKTF::CancelTune()
{
    StdFunc::Cancel();
}

void TuneDialogKTF::RefreshAnalogValues(int bdnum)
{
    switch (bdnum)
    {

    case CKTF_BDA_IN: // Блок #1
        FillBd1(this);
        break;
...
int TuneDialogKTF::TunePt100Channel()
{
    QGridLayout *glyout = new QGridLayout;
    QVBoxLayout *vlyout = new QVBoxLayout;
    QLabel *lbl = new QLabel("Количество усреднений");
...
void TuneDialogKTF::CalcNewPt100Coefs()
{
...
int TuneDialogKTF::AskTempMinus20()
{
...
void TuneDialogKTF::CalcTempCorCoefs()
{
    //int i;
...
void TuneDialogKTF::GenerateReport()
{
