...
Раздел описывает сигнализацию для подключённого устройства.
...
<alarms desc="Сигнализация">
    ...
    <state-all desc="Состояние устройства">...</state-all>
</alarm>
```

Нода alarms может иметь секции следующих типов: *critical*, *info*, *warning* или *state-all*. Данные ноды содержат следующие элементы:
...
Элементы ноды *state-all* также должны содержать элементы ***type*** для указания типа сигнализации в окне "Состояние устройства".
...
Системный журнал имеет одну и ту же структуру в любом из модулей, считается, что данный журнал всегда есть в модуле, поэтому он не описывается в XML.
...
Если любой из журналов отсутствует в модуле (например, журнал измерений в модулях АВ-ТУК), соответствующий элемент (например, *meas*) должен отсутствовать в разделе *journals* или быть пустым.
...
Раздел, описывающий отображение диалога "Секретные операции" для АВМ-Наладки.

```xml
<hidden desc="Секретные операции">
    <hidden-tab desc="..." background="..." prefix="..." flag="...">
        <mwidget view="..." title="...">
            <name>...</name>
            <type>...</type>
            <addr>...</addr>
...
Диалог "Секретные операции" может быть представлен в виде последовательности вкладок, на которых расположены виджеты для ввода данных определённого типа. При нажатии кнопки "Записать" формируется структура по описанным виджетам и отправляется в устройство.

* Элемент ***hidden-tab*** - задаёт настройки вкладки и её содержимое (список виджетов), имеет следующие обязательные атрибуты:
  * атрибут *desc* -  определяет название вкладки;
  * атрибут *background* - указывает путь до файла изображения из ресурсов ПО для его отрисовки на заднем фоне данной вкладки;
  * атрибут *prefix* - определеяет префикс имени вкладки (для поиска вкладки внутри диалога);
  * атрибут *flag* - флаг битовой сборки данных текущей вкладки, используется внутри отправляемой структуры;
* Элемент ***mwidget*** задаёт виджет для текущей вкладки, имеет следующие обязательные атрибуты:
  * атрибут *view* - определяет тип виджета для ввода данных;
  * атрибут *title* - задаёт текст на элементе типа *Label* рядом с отображаемым виджетом;
* Элемент ***name*** - задаёт имя виджета для его поиска внутри диалога с помощью системы Qt, **(!!!) имена не должны повторяться или быть пустыми**;
* Элемент ***type*** - определяет тип данных, получаемый от виджета во время отправки блока данных в устройство **(по умолчанию используется тип *uint32*)**;
* Элемент ***addr*** - задаёт адрес данных, необходимых для запроса к устройству, используется для заполнения виджета;
* Элемент ***index*** - задаёт индекс прочитанных из виджета данных внутри отправляемого в устройство блока данных;
* Элемент ***visibility*** - задаёт видимость виджета, используется для того, чтобы на вкладке не отображались виджеты, описывающие зарезервированные поля в отправляемой структуре данных.

#### BSI Ext

Раздел, описывающий структуру блока BSI Ext (англ. extended - расширенный), а также аспекты его отображения в диалоге "О приборе".

```xml
<bsi-ext desc="BSI Ext">
    <item>
        <addr>...</addr>
        <desc>...</desc>
        <type>...</type>
...
Структуру блока BSI Ext можно описать с помощью простой последовательности элементов, представляемых элементами типа *item*.

* Элементы ***item*** - описывают один элемент блока BSI Ext;
* Элемент ***addr*** - задаёт адрес элемента блока BSI Ext;
* Элемент ***desc*** - задаёт описание элемента блока BSI Ext, используется в диалоге "О приборе" в качестве текста на элементе типа *Label* рядом с отображаемым виджетом;
* Элемент ***type*** - определяет тип данных, хранимых указанным элементом в блоке BSI Ext;
* Элемент ***visibility*** - задаёт видимость виджета для отображения элемента блока BSI Ext в диалоге "О приборе".
