...
public:
    ...
    typedef struct
    {
        quint32 Eq_type;   //тип контролируемого оборудования: 0 - асинхронный двигатель, 1- трансформатор, 2 - реактор
        ...
        quint32 DDosc;	   // Разрешение запуска осциллограммы
                                                                         // áèò 0 - îò ÖÊÓ, áèò 1 - ïî ñèãíàëó PD1, áèò 2 - ïî ñêà÷êó ðåæèìà.
        ...
        float DUosc;	 // Уставка скачка напряжения для запуска осциллографирования - % от номинала
        float DIosc;	 // Уставка скачка тока для запуска осциллографирования -  % от I2nom
        float	DUImin;	 // Уставка порога минимального напряжения - % от номинального уровня
        float	Imin;	 // Уставка порога минимального тока - % от номинального уровня
        float   TNNTdop; // Предельно допустимая температура ННТ в°С
        float	TNNTpred; // Уставка предупредительной сигнализации по температуре ННТ в °С
        ...
        float	Umax;	  // Максимальное измеряемое фазное напряжение на входе прибора, В эфф (не более 305В)
        float	Imax;	  // Максимальный измеряемый ток на входе прибора, А эфф  (не более 33А)
        ...
        float Kvibr;		// Коэффициент передачи датчиков вибрации
                                            // для типа 0 - в В/(м/с/с), для типа 1 - в В/(мм/с), для типа 2 - в В/мкм.
        ...
        float VVibrA_pred; // предупредительная уставка по скорости роста СКЗ виброускорения, м/с/с
        float VVibrV_pred; // предупредительная уставка по скорости роста СКЗ виброскорости, мм/с
        float VVibrD_pred; // предупредительная уставка по скорости роста СКЗ виброперемещения, мкм
        float VVibrA_alarm; // аварийная уставка по скорости роста СКЗ виброускорения, м/с/с
        float VVibrV_alarm; // аварийная уставка по скорости роста СКЗ виброскорости, мм/с
        float VVibrD_alarm; // аварийная уставка по скорости роста СКЗ виброперемещения, мкм
        ...
        quint32 VibroType;	// Тип вибродатчика: 0 - виброускорение, 1 - виброскорость, 2 - виброперемещение.
        ...
        quint32 TdatNum;    // Количество датчиков температуры обмоток, подключенных по Modbus Master
