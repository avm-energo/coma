...
Config21::Config21(QVector<publicclass::DataRec> &config, bool BaseBoard)
{
    ...
    quint32 InputsNum = Config2x::ModTypeMap().value(Type).AInSize;
    ...
    if (StartInIndex != 0)
    {
        config.append({StartInIndex, InputsNum * sizeof(qint8), Bci_block.inblk.in_type.data()});
        config.append({StartInIndex+1, InputsNum * sizeof(float), Bci_block.inblk.in_min.data()});
        config.append({StartInIndex+2, InputsNum * sizeof(float), Bci_block.inblk.in_max.data()});
        config.append({StartInIndex+3, InputsNum * sizeof(float), Bci_block.inblk.in_vmin.data()});
        config.append({StartInIndex+4, InputsNum * sizeof(float), Bci_block.inblk.in_vmax.data()});
        config.append({StartInIndex+5, InputsNum * sizeof(float), Bci_block.inblk.setminmin.data()});
        config.append({StartInIndex+6, InputsNum * sizeof(float), Bci_block.inblk.setmin.data()});
        config.append({StartInIndex+7, InputsNum * sizeof(float), Bci_block.inblk.setmax.data()});
        config.append({StartInIndex+8, InputsNum * sizeof(float), Bci_block.inblk.setmaxmax.data()});
        config.append({StartInIndex+9, InputsNum * sizeof(qint8), Bci_block.inblk.osc.data()});
