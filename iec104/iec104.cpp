    ...
    while (!ThreadMustBeFinished)
    {
        ...
        if (!FileSending)
        {
            if (!InputQueue->isEmpty())
            {
                ...
                {
                    //                    QVector<S2::DataRec> *ptr =
                    //                    reinterpret_cast<QVector<S2::DataRec>
...
int IEC104Thread::isIncomeDataValid(QByteArray ba)
{
    try
    {
        ...
        case I104_U:
        {
            ...
            if ((baat2 == I104_STARTDT_CON) && (Command == I104_STARTDT_ACT)) // если пришло подтверждение старта и
                                                                              // перед этим мы старт запрашивали
            {
                Timer104->stop();
                Command = I104_STARTDT_CON;
                SendGI();
            }
            if ((baat2 == I104_STOPDT_CON) && (Command == I104_STOPDT_ACT)) // если пришло подтверждение стопа и
                                                                            // перед этим мы стоп запрашивали
                Command = I104_STOPDT_CON;
            if ((baat2 == I104_TESTFR_CON) && (Command == I104_TESTFR_ACT)) // если пришло подтверждение теста и
                                                                            // перед этим мы тест запрашивали
