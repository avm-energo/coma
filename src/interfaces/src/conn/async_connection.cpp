...
{
    if (std::holds_alternative<DataTypes::GeneralResponseStruct>(resp))
        emit responseSend(std::get<DataTypes::GeneralResponseStruct>(
            resp)); // to distinguish end of operation or error from long requests
