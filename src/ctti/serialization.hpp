...
#include <array>
#include <detail/nlohmann_json.hpp>
#include <model.hpp>
#include <symbol.hpp>
#include <tuple>
...
namespace serialization
{
    ...
    template <typename Formatter, typename Output> struct Writer
    {
        ...
        template <typename T> void raw_value(const T &value)
        {
        ...
        template <typename T> void begin_object(const T &object)
        {
            _formatter.begin_object(*_output, object);
        }

        template <typename T> void end_object(const T &object)
        {
    ...
    {
        template <typename Output, typename T> void raw_value(Output &output, const T &value)
        {
            output.write(value);
        }

        template <typename Output> void value_separator(Output &out)
        {
            out.write(", ");
        }

        template <typename Output, typename T> void begin_object(Output &out, const T &)
        {
            out.write("{");
        }

        template <typename Output, typename T> void end_object(Output &out, const T &)
        {
            out.write("}");
        }
    ...
    struct enum_from_string_reader
    {
        ...
        template <typename T, typename Callback> void enum_value(T &value, Callback callback)
        {
    ...
    namespace detail
    {
        ...
        template <typename T>
        struct is_default_ostream_serializable<T, false,
            ctti::meta::void_t<decltype(std::declval<T>().ctti_ostream_print())>> : public ctti::meta::true_
        {
        };

        template <typename T>
        struct is_default_ostream_serializable<T, false,
            ctti::meta::void_t<decltype(ctti_ostream_print(std::declval<ctti::type_tag<T>>()))>>
            : public ctti::meta::true_
        {
        };
    }
...
{
    ctti::serialization::serialize(
        ctti::serialization::json_formatter(), ctti::serialization::ostream_otuput(os), value);
    return os;
}
