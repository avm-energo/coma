...
ConfKDVDialog::ConfKDVDialog(ConfigKDV *ckdv, QWidget *parent) : AbstractConfDialog(parent)
{
    m_list = { CType, Abs_104, Cycle_104, T1_104, T2_104, T3_104, k_104, w_104, IP_ID, Mask_ID, GW_ID, Port_ID, SNTP_ID,
        Baud_ID, Parity_ID, stopbit_ID, adrMB_ID, Unom1, U2nom, ITT1nom_KTF_KDV, ITT2nom_KTF_KDV, Iwnom, Tamb_nom,
        dTNNTnom, Kdob, TauWnom, Umaxm, Imaxm, TdatNum, Fnom, nom_slip, UVmax, DUosc, DIosc_ID, DUImin_ID, Imin,
        TNNTpred, TNNTdop, VibrA_pred, VibrA_alarm, VibrV_pred, VibrV_alarm, VibrD_pred, VibrD_alarm, VVibrA_pred,
        VVibrA_alarm, VVibrV_pred, VVibrV_alarm, VVibrD_pred, VVibrD_alarm, GTnnt, GOvc, NFiltr_ID, T_Data_Rec,
        NHarmFilt_ID, OscPoints, Kvibr, NumA_KDV, Poles, Stator_Slotes, Rotor_bars, VibroType, Sensors, MBMaster,
        MBMab1, MBMab2, MBMab3, MBMab4, RTerm, W100, Trele_pred, Trele_alarm, DDOsc_ID };
    ConfKDV = ckdv;
}
...
//        row, 1, 1, 1);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::Imaxm), 0, 10000, 1), row, 2, 1, 3);
//    row++;
...
//        row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::DUosc), 0, 10000, 1), row, valueColumn);

//    row++;
//    gridlyout->addWidget(
//        WDFunc::NewLBL2(this, "Уставка скачка тока для запуска осциллографирования -  % от I2nom:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::DIosc_ID), 0, 10000, 1), row, valueColumn);

//    row++;
//    gridlyout->addWidget(
//        WDFunc::NewLBL2(this, "Уставка порога минимального напряжения - % от номинального уровня:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::DUImin_ID), 0, 1000, 1), row, valueColumn);

//    row++;
//    gridlyout->addWidget(
//        WDFunc::NewLBL2(this, "Уставка порога минимального тока - % от номинального уровня:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::Imin), 0, 10000, 1), row, valueColumn);
//    row++;

//    gridlyout->addWidget(
//        WDFunc::NewLBL2(this, "Предупредительная уставка сигнализации по температуре ННТ в °С:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::TNNTpred), 0, 10000, 1), row, valueColumn);
//    row++;

//    gridlyout->addWidget(
//        WDFunc::NewLBL2(this, "Аварийная уставка сигнализации по температуре ННТ в °С:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::TNNTdop), 0, 10000, 1), row, valueColumn);
//    row++;
//    gb->setLayout(gridlyout);
//    return gb;
//}
...
//    gridlyout->addWidget(new QLabel("Предупредительные", this), row, valueColumn);
//    gridlyout->addWidget(new QLabel("Аварийные", this), row, extraColumn);
//    row++;

//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Уставка по СКЗ виброускорения, м/с/с:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VibrA_pred), 0, 1000, 1), row, valueColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VibrA_alarm), 0, 1000, 1), row, extraColumn);

//    row++;
//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Уставка по СКЗ виброскорости, мм/с:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VibrV_pred), 0, 10000, 1), row, valueColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VibrV_alarm), 0, 10000, 1), row, extraColumn);

//    row++;
//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Уставка по СКЗ виброперемещения, мкм:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VibrD_pred), 0, 10000, 1), row, valueColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VibrD_alarm), 0, 10000, 1), row, extraColumn);
...
//        row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VVibrA_pred), 0, 1000, 1), row, valueColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VVibrA_alarm), 0, 1000, 1), row, extraColumn);
...
//                             "виброскорости, мм/с:"),
//        row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VVibrV_pred), 0, 10000, 1), row, valueColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VVibrV_alarm), 0, 10000, 1), row, extraColumn);
...
//        row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VVibrD_pred), 0, 10000, 1), row, valueColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VVibrD_alarm), 0, 10000, 1), row, extraColumn);
//    gb->setLayout(gridlyout);
//    return gb;
//}
...
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::GTnnt), 0, 10000, 3), row, valueColumn);

//    row++;
//    gridlyout->addWidget(
//        WDFunc::NewLBL2(this, "Гистерезис сигнализации по токовой перегрузке, % от Iwnom:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::GOvc), 0, 10000, 1), row, valueColumn);
...
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::NFiltr_ID), 0, 10000, 0), row, valueColumn);

//    row++;
//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Интервал записи данных в ПЗУ (тренд), в секундах:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::T_Data_Rec), 0, 10000, 0), row, valueColumn);

//    row++;
//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Постоянная времени фильтрации гармоник:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::NHarmFilt_ID), 0, 10000, 0), row, valueColumn);
...
//    gridlyout->addLayout(hlyout, row, valueColumn);

//    row++;
//    gridlyout->addWidget(
//        WDFunc::NewLBL2(this, "Количество точек осциллограммы на период основной частоты:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::OscPoints), 0, 10000, 0), row, valueColumn);
...
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::Kvibr), 0, 10000, 2), row, valueColumn);

//    row++;
//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Порядковый номер двигателя:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::NumA_KDV), 0, 10000, 0), row, valueColumn);

//    row++;
//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Число пар полюсов (от 1 до 10):"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::Poles), 0, 10000, 0), row, valueColumn);

//    row++;
//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Число пазов на статоре:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::Stator_Slotes), 0, 10000, 0), row, valueColumn);

//    row++;
//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Число стержней ротора:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::Rotor_bars), 0, 10000, 0), row, valueColumn);

//    row++;
//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Тип вибродатчика:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::VibroType), 0, 10000, 0), row, valueColumn);

//    row++;
//    gridlyout->addWidget(WDFunc::NewLBL2(this, "Подключенные датчики вибрации:"), row, textColumn);
//    gridlyout->addWidget(WDFunc::NewSPB2(this, nameByValue(BciNumber::Sensors), 0, 10000, 0), row, valueColumn);
