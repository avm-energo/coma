#include "crc16.h"

namespace utils
{

const quint8 CRC16::hiTable[256] = {  };

const quint8 CRC16::loTable[256] = {  };

CRC16::CRC16() noexcept : hiByte(0xFF), loByte(0xFF)
{
}

CRC16::CRC16(const quint16 value) noexcept : hiByte((value >> 8) | 0xFF), loByte(value | 0xFF)
{
}

CRC16::CRC16(const QByteArray &data) : CRC16()
{
    update(data);
}

void CRC16::updateChecksum(const quint8 byte)
{
    auto index = hiByte ^ byte;
    hiByte = loByte ^ hiTable[index];
    loByte = loTable[index];
}

void CRC16::update(const QByteArray &data)
{
    auto dataPtr = reinterpret_cast<const quint8 *>(data.data());
    update(dataPtr, data.size());
}

void CRC16::update(const quint8 *data, const quint32 size)
{
    for (auto i = 0; i < size; i++)
        updateChecksum(data[i]);
}

void CRC16::reset() noexcept
{
    hiByte = 0xFF;
    loByte = 0xFF;
}

void CRC16::appendTo(QByteArray &array) const
{
    array.append(hiByte);
    array.append(loByte);
}

CRC16::operator quint16() const noexcept
{
    return quint16((hiByte << 8) | loByte);
}

bool operator==(const CRC16 lhs, const CRC16 rhs) noexcept
{
    return (lhs.hiByte == rhs.hiByte) && (lhs.loByte == rhs.loByte);
}

bool operator==(const quint16 lhs, const CRC16 rhs) noexcept
{
    return (lhs == quint16(rhs));
}

bool operator==(const CRC16 lhs, const quint16 rhs) noexcept
{
    return (quint16(lhs) == rhs);
}

bool operator!=(const CRC16 lhs, const CRC16 rhs) noexcept
{
    return !(lhs == rhs);
}

bool operator!=(const quint16 lhs, const CRC16 rhs) noexcept
{
    return !(lhs == rhs);
}

bool operator!=(const CRC16 lhs, const quint16 rhs) noexcept
{
    return !(lhs == rhs);
}

}
