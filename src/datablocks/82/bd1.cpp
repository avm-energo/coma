...
void Bd182::setupValuesDesc()
{
    ...
    addNewGroup("Cos phi по истинной активной мощности", "Cos", 27, 3, &m_blockData->CosPhiNat[0], 4);
    addNewGroup("Активная мощность по 1-й гармонике", "PF", 30, 3, &m_blockData->Pf[0], 3);
    addNewGroup("Полная мощность по 1-й гармонике", "SF", 33, 3, &m_blockData->Sf[0], 3);
    addNewGroup("Реактивная мощность по 1-й гармонике", "QF", 36, 3, &m_blockData->Qf[0], 3);
    addNewGroup("Cos phi по 1-й гармонике", "CosPhi", 39, 3, &m_blockData->CosPhi[0], 4);
    addNewGroup("Угол между током и напряжением", "PHI", 42, 3, &PHI[0], 4);
}
