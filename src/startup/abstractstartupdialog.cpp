...
void AbstractStartupDialog::WriteCor()
{
    ...
    Connection::iface()->writeCommand(Commands::C_WriteUserValues, values);
...
void AbstractStartupDialog::GetCorBd()
{
    ...
    Connection::iface()->reqStartup(m_startupBlockDescription.initStartRegAdr,
...
void AbstractStartupDialog::SetupCor()
{
    if (checkPassword())
    {
        if (checkStartupValues())
        {
            ...
            Connection::iface()->writeCommand(Commands::C_SetStartupValues);
            GetCorBd();
        }
    }
}
...
void AbstractStartupDialog::ResetCor()
{
    if (checkPassword())
    {
        ...
        Connection::iface()->writeCommand(Commands::C_ClearStartupValues);
...
void AbstractStartupDialog::sendCommand(Commands cmd, bool value)
{
    if (checkPassword())
    {
        Connection::iface()->writeCommand(cmd, value);
        GetCorBd();
    }
}
