...
Tune21One::Tune21One(int tuneStep, Device::CurrentDevice *device, QWidget *parent)
    : AbstractTuneDialog(tuneStep, device, parent)
    , m_bac(new Bac21(this))
    , m_bda(new Bda21(this))
{
...
void Tune21One::setTuneFunctions()
{
    ...
    addTuneFunc("Отображение предупреждения...", &Tune21One::showPreWarning);
    addTuneFunc("Запрос настроечных параметров...", &AbstractTuneDialog::readTuneCoefs);
    addTuneFunc("Проверка настроечных параметров...", &Tune21One::checkTuneCoefs);
    addTuneFunc("Задание режима конфигурирования модуля...", &Tune21One::setSMode2);
    if (m_tuneStep == KIVTS_ADCU)
    {
        addTuneFunc("Регулировка...", &Tune21One::ADCCoef1);
        addTuneFunc("Отображение диалога задания входных данных...", &Tune21One::showEnergomonitorInputDialog);
    }
    if (m_tuneStep == KIVTS_ADCI)
    {
        addTuneFunc("Регулировка для Кацп = 1...", &Tune21One::ADCCoef1);
        addTuneFunc("Отображение диалога задания входных данных...", &Tune21One::showEnergomonitorInputDialog);
        addTuneFunc("Регулировка для Кацп = 2...", &Tune21One::ADCCoef2);
        addTuneFunc("Отображение диалога задания входных данных...", &Tune21One::showEnergomonitorInputDialog);
        addTuneFunc("Регулировка для Кацп = 4...", &Tune21One::ADCCoef4);
        addTuneFunc("Отображение диалога задания входных данных...", &Tune21One::showEnergomonitorInputDialog);
        addTuneFunc("Регулировка для Кацп = 8...", &Tune21One::ADCCoef8);
        addTuneFunc("Отображение диалога задания входных данных...", &Tune21One::showEnergomonitorInputDialog);
        addTuneFunc("Регулировка для Кацп = 16...", &Tune21One::ADCCoef16);
        addTuneFunc("Отображение диалога задания входных данных...", &Tune21One::showEnergomonitorInputDialog);
        addTuneFunc("Регулировка для Кацп = 32...", &Tune21One::ADCCoef32);
        addTuneFunc("Отображение диалога задания входных данных...", &Tune21One::showEnergomonitorInputDialog);
        addTuneFunc("Регулировка канала Tmk0...", &Tune21One::Tmk0);
    }
    addTuneFunc("Запись настроечных коэффициентов и восстановление конфигурации...", &Tune21One::SendBac);
    addTuneFunc("Проверка регулировки...", &Tune21One::CheckTune);
}

Error::Msg Tune21One::showPreWarning()
{
...
Error::Msg Tune21One::setSMode2()
{
    m_async->writeCommand(Interface::Commands::C_SetMode, 0x02);
    return Error::Msg::NoError;
}

Error::Msg Tune21One::ADCCoef(int coef)
{
...
Error::Msg Tune21One::setADCCoef(const int coef)
{
...
Error::Msg Tune21One::showRetomDialog(int coef)
{
    QWidget *w = new QWidget(this);
...
void Tune21One::IULayout(RegType type, int coef, QVBoxLayout *lyout)
{
    struct retomStruct
    {
        QString I;
...
bool Tune21One::checkBdaIn(int current)
{
#ifdef NO_LIMITS
    return true;
#endif
...
Error::Msg Tune21One::showEnergomonitorInputDialog()
{
    ...
    connect(popup, &EEditablePopup::accepted, this, &Tune21One::CalcTuneCoefs);
    connect(popup, &EEditablePopup::cancelled, [] { return Error::GeneralError; });
    popup->execPopup();
    return Error::Msg::NoError;
}

void Tune21One::CalcTuneCoefs()
{
