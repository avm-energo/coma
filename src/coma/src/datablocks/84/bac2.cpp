...
Bac284::Bac284(QObject *parent) : DataBlock(parent)
{
    m_blockData = std::unique_ptr<BlockData>(new BlockData);
    setBlock({ 2, "Настроечные параметры", DataTypes::DataBlockTypes::BacBlock, //
        m_blockData.get(), sizeof(BlockData), true });
}

void Bac284::setupValuesDesc()
{
...
void Bac284::setDefBlock()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        m_blockData->KmU[i] = 1.0f;
        m_blockData->KmI1[i] = 1.0f;
        m_blockData->KmI16[i] = 1.0f;
        m_blockData->KmI2[i] = 1.0f;
        m_blockData->KmI32[i] = 1.0f;
        m_blockData->KmI4[i] = 1.0f;
        m_blockData->KmI8[i] = 1.0f;
        m_blockData->N1_TT[i] = 10.0f;
        m_blockData->TKPsi_a[i] = 0.0f;
        m_blockData->TKPsi_b[i] = 0.0f;
    }
}

Bac284::BlockData *Bac284::data()
{
