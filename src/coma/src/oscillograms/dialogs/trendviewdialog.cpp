...
SignalChooseWidget *TrendViewDialog::createSignalChooseWidget(const TrendViewDialog::Signals &sig)
{
...
void TrendViewDialog::setupUI()
{
    ...
    if (!digital.noSignals())
    {
        ...
        vlyout->addWidget(createSignalChooseWidget(digital));
    }
    if (!analog.noSignals())
    {
        vlyout->addWidget(createToolBar(ST_ANALOG));
        vlyout->addWidget(createSignalChooseWidget(analog));
    }
...
void TrendViewDialog::graphClicked(QCPAbstractPlottable *plot, int dataIndex)
{
    ...
    auto name = plot->name();
    //    auto message = QString("Clicked on graph '%1' at data point #%2 with position X:%3, Y:%4.")
    //                       .arg(name)
    //                       .arg(dataIndex)
    //                       .arg(x)
    //                       .arg(y);
...
void TrendViewDialog::setModel(TrendViewModel *model)
{
    ...
    {
        m_trendModel = model;
        sizeX.min = model->xmin();
        sizeX.max = model->xmax();
        sizeY.min = model->ymin();
        sizeY.max = model->ymax();
