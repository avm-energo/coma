...
iec104::iec104(QObject *parent) : QObject(parent)
{
    ...
    connect(Parse, SIGNAL(signalsreceived(Parse104::Signals104 &)), this, SIGNAL(signalsready(Parse104::Signals104 &)),
...
int Parse104::isIncomeDataValid(QByteArray ba)
{
    ...
    case I104_U:
    {
        if ((ba.at(2) == I104_STARTDT_CON)
            && (cmd == I104_STARTDT_ACT)) // если пришло подтверждение старта и перед этим мы старт запрашивали
            cmd = I104_STARTDT_CON;
        if ((ba.at(2) == I104_STOPDT_CON)
            && (cmd == I104_STOPDT_ACT))  // если пришло подтверждение стопа и перед этим мы стоп запрашивали
            cmd = I104_STOPDT_CON;
        if ((ba.at(2) == I104_TESTFR_CON)
            && (cmd == I104_TESTFR_ACT))  // если пришло подтверждение теста и перед этим мы тест запрашивали
            cmd = I104_TESTFR_CON;
        return I104_RCVNORM;
        break;
    }
