...
void Coma::StartWork()
{
    if (!Reconnect)
    {
        ...
        connect(dlg, SIGNAL(Accepted(ConnectDialog::ConnectStruct *)), this,
            SLOT(SetConnection(ConnectDialog::ConnectStruct *)));
...
void Coma::New104()
{
    ...
    connect(Ch104, SIGNAL(Sponsignalsready(IEC104Thread::SponSignals *)), this,
        SLOT(UpdateStatePredAlarmEvents(IEC104Thread::SponSignals *)));
...
void Coma::NewModbus()
{
    ...
    connect(
        ChModbus, SIGNAL(CoilSignalsReady(ModBus::Coils)), this, SLOT(ModbusUpdateStatePredAlarmEvents(ModBus::Coils)));
...
void Coma::PredAlarmState()
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        hlyout = new QHBoxLayout;
        hlyout->addWidget(
            WDFunc::NewLBL(w, "", "", QString::number(3027 + i), (PredAlarmEvents[13 + i]) ? pmred : pmgrn));
